...
PREPROCESS = True # if True, applies filter(s) and removes DC offset
...
REF_FREQ_MAP = {
    ...
    'D': {*np.around(np.arange(6, 13.001, 0.23), decimals=2)}
}

FPATH_DOTENV = '../software/speller/data_collection_platform/backend/.env'
DPATH_DATA = '.'
...
def get_subject_data(database_url, subject_id, bad_collection_ids=[], verbose=True):
    '''
    Downloads a subject's runs from a database and selects those with the target frequency type.
    Returns a dictionary of lists of pandas dataframes (one for each data collection run).
    The keys of the dictionary are the frequency types.
    Ex: {'A': [df1, df2], 'B': [df3, df4]}
    '''
    ...
    with alchemy_engine.connect() as db_connection:
        ...
        subject_ids = {
            ...
            for id in all_ids 
            if (
                (subject_id.lower() in id.lower()) and 
                (id not in BAD_SUBJECT_IDS)
            )}
        if verbose:
        ...
        for collection_id in collection_ids:
            ...
            if freq_type not in subject_data:
                subject_data[freq_type] = []

            # if the dataframe is not empty
            if n_rows_per_character.sum() > 0:
                subject_data[freq_type].append(df_run)
        ...
        if verbose:
            number_of_runs = {freq_type: len(runs) for freq_type, runs in subject_data.items()}
...
def get_data(database_url, subject_id):

    # get data from sessions of interest (one dataframe per session)
    subject_data_all = get_subject_data(database_url, subject_id, 
        bad_collection_ids=BAD_COLLECTION_IDS, verbose=VERBOSE)

    for freq_type, subject_data in subject_data_all.items():
        ...
        for i_run, run_data in enumerate(subject_data):
            ...
            for freq in freqs:
                ...
                trial_data_preprocessed = trial_data.to_numpy()
                if PREPROCESS:
                    trial_data_preprocessed = preprocess_trial(trial_data_preprocessed, FS)
        ...
        all_blocks = np.array(all_blocks).T
        if VERBOSE:
            print(f'Final matrix shape for frequency type {freq_type}: {all_blocks.shape}')
        ...
        # save to .pkl file
        subdir_name = 'preprocessed' if PREPROCESS else 'raw'
        fpath_out = Path(DPATH_DATA, subdir_name, subject_id, f'{subject_id}_type{freq_type}.pkl')
        fpath_out.parents[0].mkdir(parents=True, exist_ok=True)
        to_save = {'data': all_blocks, 'freq_type': freq_type, 'freqs': freqs, 'chars': chars}
...
if __name__ == '__main__':
    ...
    subject_ids = [f'S{i:02}' for i in range(1, 11)] + ['P02', 'P10']

    for subject_id in subject_ids:
        print(f'========== {subject_id} ==========')
        get_data(database_url, subject_id)

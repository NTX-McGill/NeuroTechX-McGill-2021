...
def filter_bank_cca_it(signal, fund_freq, lower_freq, upper_freq,
                       num_harmonics, template, sampling_rate, num_fb=2, fb_a=1, fb_b=0.3,
                       filter_order=2, rp=0.3, onset=250, end=50, *args, **kwargs):
    sum = []
    ...
    for i in range(1, num_fb+1):
        ...
        rho = standard_cca_it_cca(filter_bank[onset:-end, :],
                                  sampling_rate=sampling_rate, fund_frequency=fund_freq*i,
                                  num_harmonics=num_harmonics, template=template[onset:-end, :])
